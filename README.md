...
| **Cross-language** | A multilingual embedding model runs locally through ONNX (statically linked, downloaded once to `~/.tabularium/models`). Ask in Russian, find what was saved in English -- real, but with a measured quality gap versus same-language recall, not parity (`evals/paper/skeleton.md` §3.5). Without the model the engine degrades to lexical recall and says so. |
